...
Board::Board() {
  ...
  this->board =
      std::vector<std::vector<uint8_t>>(8, std::vector<uint8_t>(8, EMPTY));
  for (auto &piece : this->pieces) {
    this->board[piece.square.x][piece.square.y] = piece.id;
  }
}
...
std::vector<Move> Board::get_moves(Colour colour) {
  std::vector<Move> moves;
  for (size_t i = 0; i < 16; i++) {
    int index = i + (colour == Colour::White ? 0 : 16);
    if (!this->pieces[index].taken) {
      std::vector<Move> piece_moves = this->pieces[index].get_moves(*this);
...
std::vector<Move> Board::get_moves_depth(Colour colour) {
  std::vector<Move> moves;
  for (size_t i = 0; i < 16; i++) {
    int index = i + (colour == Colour::White ? 0 : 16);
    if (!this->pieces[index].taken) {
...
Move Board::get_move(Square &from, Square &to, MoveType type) {
  Piece &piece = this->pieces[this->board[from.x][from.y]];
  if (this->board[to.x][to.y] != EMPTY) {
    Piece &takes = this->pieces[this->board[to.x][to.y]];
    return Move(piece, takes, from, to, type);
  }
  return Move(piece, std::nullopt, from, to, type);
}
...
bool Board::is_check(Colour colour) {
  std::vector<Move> moves = this->get_moves(opposite(colour));
  for (auto &move : moves) {
    if (move.takes != std::nullopt && move.takes->type == PieceType::King &&
        move.takes->colour == colour) {
      return true;
    }
  }
  return false;
